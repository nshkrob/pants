...
class PushDb(object):
  ...
  class Entry(object):
    ...
    :param Namever named_ver: The last named release of this entry (or None)
  ...
  * ``restrict_push_branches`` - Optional list of branches to restrict publishing to.
  ...
  def setup_parser(cls, option_group, args, mkflag):
    ...
    option_group.add_option(mkflag("local-snapshot"), mkflag("local-snapshot", negate=True),
                            ...
                            action="callback", callback=mkflag.set_bool,
                            help="[%%default] Iff %s is specified, publishes jars with '-SNAPSHOT' "
                                 "revision suffixes." % local_flag)
  ...
  def __init__(self, *args, **kwargs):
    ...
    if self.context.options.jar_publish_override:
      if self.named_snapshot:
        raise TaskError('Flags %s and %s are mutually exclusive!' % (named_snapshot_flag,
                                                                     override_flag))
